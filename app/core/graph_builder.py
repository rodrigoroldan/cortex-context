...
async def upsert_node(
    ...
    *,
    ...
    """
    Cria ou atualiza um nó no Neo4j com suporte a multi-labels e versionamento bitemporal.

    MERGE usa a primary_label + id como chave de upsert.
    Labels adicionais (ex: pilar Intent/System) são aplicados via SET.

    Propriedades bitemporais adicionadas automaticamente:
    - ingested_at:  sempre atualizado para datetime UTC atual
    - valid_from:   setado apenas na PRIMEIRA ingestão (COALESCE preserva valor existente)
    - commit_sha:   SHA do commit que originou esta versão (opcional)
    """
    ...
    now = _now_utc()
    bitemporal_props: dict = {"ingested_at": now}
    if commit_sha:
        bitemporal_props["commit_sha"] = commit_sha
...
async def ingest_nodes(
    ...
    *,
    ...
    for node in nodes:
        try:
            await upsert_node(driver, node, commit_sha=commit_sha)
            count += 1
...
async def upsert_chunk(driver: AsyncDriver, chunk: NodeData) -> None:
    ...
    Propriedades bitemporais (ingested_at, valid_from) são aplicadas como nos nós normais.
